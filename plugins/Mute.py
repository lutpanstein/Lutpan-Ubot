...
from Lutpan.dB.mute_db import is_muted, mute, unmute
from Lutpan.fns.admins import ban_time

from . import asst, eod, get_string, inline_mention, lutpan_bot, lutpan_cmd
...
async def startmute(event):
    ...
    if input_ := event.pattern_match.group(1).strip():
        ...
        if reply.out or userid in [lutpan_bot.me.id, asst.me.id]:
