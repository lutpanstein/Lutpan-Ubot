...
if udB.get_key("PMSETTING"):
        ...
        if not is_approved(user.id) and event.text != UND:
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm:
                        results = await lutpan.bot.inline_query(
                    ...
                    if inline_pm:
                        try:
                            results = await lutpan_bot.inline_query(
                                my_bot, f"ip_{user.id}"
                            )
                            _to_delete[user.id] = await results[0].click(
                                user.id, reply_to=event.id, hide_via=True
    ...
    @lutpan_cmd(pattern="(start|stop|clear)archive$", fullsudo=True)
    async def _(e):
    ...
    @lutpan_cmd(pattern="(a|ok)(?: |$)", fullsudo=True)
    ...
    @lutpan_cmd(pattern="(no|tolak)(?: |$)", fullsudo=True)
...
@lutpan_cmd(pattern="block( (.*)|$)", fullsudo=True)
...
@lutpan_cmd(pattern="unblock( (.*)|$)", fullsudo=True)
...
@lutpan_cmd(pattern="listapproved$", owner=True)
async def list_approved(event):
    ...
    for i in all:
        try:
            name = get_display_name(await lutpan_bot.get_entity(i))
        except BaseException:
            name = ""
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await lutpan_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user = await lutpan_bot.get_entity(uid)
        except BaseException:
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await lutpan_bot(BlockRequest(uid))
    except BaseException:
        pass
    try:
        user = await lutpan_bot.get_entity(uid)
...
async def in_pm_ans(event):
    ...
    await event.answer(res, switch_pm="• Lutpan •", switch_pm_param="start")


@callback(re.compile("admin_only(.*)"), from_users=[lutpan_bot.uid])
