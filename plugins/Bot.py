...
def LUTPANPIC():
    return inline_pic() or choice(LUTPAN_IMAGES)
...
async def _(event):
    ...
    file = f"lutpan{sys.argv[-1]}.log" if len(sys.argv) > 1 else "lutpan.log"
    if opt == "heroku":
        ...
        await event.reply("**Lutpan Logs.**", file=file)
    elif opt == "open":
        with open("lutpan.log", "r") as f:
            file = f.read()[-4000:]
...
async def inline_alive(ayra):
    ...
    kk = f"<a href={rep}>{y}</a>"
    als = in_alive.format(
        header, f"{lutpan_version} [{HOSTED_ON}]", LutpanVer, pyver(), uptime, kk
    )
