...
from Lutpan.dB.warn_db import add_warn, reset_warn, warns

from . import eor, get_string, inline_mention, udB, lutpan_cmd
...
async def warn(e):
    ...
    if count + 1 >= number:
        if "ban" in action:
            try:
                await lutpan_bot.edit_permissions(e.chat_id, user, view_messages=False)
            except BaseException:
...
@lutpan_cmd(pattern="setwarn( (.*)|$)", manager=True)
async def warnset(e):
