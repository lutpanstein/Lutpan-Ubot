...
from Lutpan import _ignore_eval
...
try:
    ...
    from Lutpan.fns.tools import safe_load
try:
...
@lutpan_cmd(pattern="bash", fullsudo=False, only_devs=False)
async def _(event):
...
@lutpan_cmd(pattern="eval", fullsudo=False, only_devs=False)
async def _(event):
    try:
        cmd = event.text.split(maxsplit=1)[1]
    except IndexError:
    ...
    if (
        ...
        and not event.out
        and event.sender_id != lutpan_bot.uid
    ):
...
@lutpan_cmd(pattern="cpp", only_devs=False)
async def doie(e):
    ...
    if m[1]:
        o_cpp += f"\n\n**• Error :**\n`{m[1]}`"
        if len(o_cpp) > 3000:
            os.remove("cpp-lutpan.cpp")
            if os.path.exists("CppLutpan"):
                os.remove("CppLutpan")
