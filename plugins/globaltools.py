...
from Lutpan.dB.gban_mute_db import (
    gban,
    gmute,
    is_gbanned,
    is_gmuted,
...
async def _(e):
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await lutpan_bot.get_entity(user)
        except BaseException:
            return await e.eor(f"`No User Found Regarding {user}`", time=5)
        ...
        rank = "Not AdMin"
        c = 0
        async for x in lutpan_bot.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@lutpan_cmd(pattern="ungban( (.*)|$)", fullsudo=True)
async def _(e):
...
@lutpan_cmd(pattern="gban( (.*)|$)", fullsudo=True)
async def _(e):
    xx = await e.eor("`BEJIRRRRR DIGBAN GA TUUUHHH WKWKWKWK...`")
    reason = ""
    if e.reply_to_msg_id:
        userid = (await e.get_reply_message()).sender_id
        try:
...
@lutpan_cmd(pattern="g(admin|)cast( (.*)|$)", fullsudo=True)
...
@lutpan_cmd(pattern="gucast( (.*)|$)", fullsudo=True)
async def gucast(event):
    ...
    kk = await event.eor("`GATAUUUU BANG KEKIRIMM KE BERAPA ORANG NTAR.`")
    er = 0
    done = 0
    if event.client._dialogs:
        dialog = event.client._dialogs
...
@lutpan_cmd(pattern="gkick( (.*)|$)", fullsudo=True)
async def gkick(e):
    xx = await e.eor("`WADDUUUUHHH DIKICK DIBERAPA GC TU BANG`")
    if e.reply_to_msg_id:
        userid = (await e.get_reply_message()).sender_id
    elif e.pattern_match.group(1).strip():
...
@lutpan_cmd(pattern="gmute( (.*)|$)", fullsudo=True)
async def _(e):
    ...
    if is_gmuted(userid):
        return await xx.eor("`Udah ga diglobal emutttt anjiiiingggggg.`", time=4)
    if e.client._dialogs:
        dialog = e.client._dialogs
    else:
        dialog = await e.client.get_dialogs()
...
@lutpan_cmd(pattern="ungmute( (.*)|$)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    gbanned_users = f"<strong>BEJIRRRRR NI ORANGGGG MAENNN GIIIBENNN FAKKK KATA LUTPAN {OWNER_NAME}</strong>:\n\n{msg}"
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"List of users GBanned by {inline_mention(lutpan_bot.me)}",
        )
...
@lutpan_cmd(pattern="gblacklist$")
async def blacklist_(event):
    await gblacker(event, "add")


@lutpan_cmd(pattern="ungblacklist$")
