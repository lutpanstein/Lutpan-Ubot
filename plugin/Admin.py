...
@lutpan_cmd(pattern="pin$", manager=True, require="pin_messages", fullsudo=True)
async def pin(msg):
    if not msg.is_reply:
...
@lutpan_cmd(pattern="purge( (.*)|$)", manager=True, require="delete_messages")
...
@lutpan_cmd(pattern="pinned", manager=True, groups_only=True)
