...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="Lutpan Help Menu", text=text, buttons=_main_help_menu
        )
...
async def help_func(ayra):
    ...
    text = _strings.get(key, "").format(lutpan, len(HELP.get(key)))
