...
from . import Carbon, eor, get_string, inline_mention, os, lutpan_cmd
...
async def crbn(event):
    ...
    xx = await Carbon(code=code, file_name="lutpan_carbon", backgroundColor=col)
    await xxxx.delete()
    await event.reply(
...
@lutpan_cmd(pattern="rayso")
async def pass_on(kaz):
    spli = kaz.text.split()
    theme, dark, title, text = None, True, get_display_name(lutpan.chat), None
    if len(spli) > 2:
        ...
        if spli[1] in RaySoTheme:
            ...
            await lutpan.eor(text)
            return
        else:
            try:
                text = lutpan.text.split(maxsplit=1)[1]
            except IndexError:
                pass
    if not theme:
