...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "©↻Lutpan"
            await eod(ok, f"✓ `Lutpan - Installed`: `{plug}` ✓\n\n{output}")
    ...
    async def heroku_logs(event):
        ...
        ok = app.get_log()
        with open("lutpan-heroku.log", "w") as log:
            log.write(ok)
        await event.client.send_file(
            event.chat_id,
            file="lutpan-heroku.log",
            thumb=LutpanConfig.thumb,
            caption="**Lutpan Heroku Logs.**",
        )

        os.remove("lutpan-heroku.log")
        await xx.delete()
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Lutpan {lutpan_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Lutpan {lutpan_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
...
async def restart(ay=None):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        try:
            Heroku = heroku3.from_key(Var.HEROKU_API)
            app = Heroku.apps()[Var.HEROKU_APP_NAME]
            if ay:
                await ay.edit("`Mulai ulang aplikasi Anda, harap tunggu sebentar!`")
            app.restart()
...
async def shutdown(ay):
    ...
    if HOSTED_ON == "heroku":
        if not (Var.HEROKU_APP_NAME and Var.HEROKU_API):
            return await ay.edit("Silahkan Isi `HEROKU_APP_NAME` and `HEROKU_API`")
        dynotype = os.getenv("DYNO").split(".")[0]
        try:
            Heroku = heroku3.from_key(Var.HEROKU_API)
            app = Heroku.apps()[Var.HEROKU_APP_NAME]
            await ay.edit("`Matikan aplikasi Anda, harap tunggu sebentar!`")
