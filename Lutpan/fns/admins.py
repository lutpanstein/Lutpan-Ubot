...
async def _callback_check(event):
    ...
    if not _lutpan_cache.get("admin_callback"):
        _lutpan_cache.update({"admin_callback": {id_: None}})
    else:
        _lutpan_cache["admin_callback"].update({id_: None})
    while not _lutpan_cache["admin_callback"].get(id_):
        await asyncio.sleep(1)
    key = _lutpan_cache.get("admin_callback", {}).get(id_)
    del _lutpan_cache["admin_callback"][id_]
    return key
...
async def get_update_linked_chat(event):
    if _lutpan_cache.get("LINKED_CHATS") and _lutpan_cache["LINKED_CHATS"].get(event.chat_id):
        ...
        if _lutpan_cache.get("LINKED_CHATS"):
            _lutpan_cache["LINKED_CHATS"].update({event.chat_id: {"linked_chat": _ignore}})
        else:
            _lutpan_cache.update(
