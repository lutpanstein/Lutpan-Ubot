...
if run_as_module:
    ...
    from ..version import __version__ as __lutpan__
    from ..version import lutpan_version

    file = f"lutpan{sys.argv[6]}.log" if len(sys.argv) > 6 else "lutpan.log"

    if os.path.exists(file):
        os.remove(file)

    HOSTED_ON = where_hosted()
    LOGS = getLogger("LutpanLogs")
    ...
    LOGS.info(f"py-Lutpan Version - {__lutpan__}")
    LOGS.info(f"Telethon Version - {__version__} [Layer: {LAYER}]")
    LOGS.info(f"Lutpan Version - {lutpan_version} [{HOSTED_ON}]")
