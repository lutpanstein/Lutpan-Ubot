...
def main():
    ...
    if (
        ...
        and lutpan_bot.run_in_loop(updater())
    ):
    ...
    lutpan_bot.run_in_loop(customize())

    # Load Addons from Plugin Channels.
    if plugin_channels:
        lutpan_bot.run_in_loop(plug(plugin_channels))

    # Send/Ignore Deploy Message..
    if not udB.get_key("LOG_OFF"):
       lutpan_bot.run_in_loop(ready())

    # Edit Restarting Message (if It's restarting)
    lutpan_bot.run_in_loop(WasItRestart(udB))
    ...
    LOGS.info(
        f"Took {time_formatter((time.time() - start_time)*1000)} to start 〄 ʟᴜᴛᴘᴀɴ ɢᴀᴄᴏʀ ᴘᴀʀᴀʜ 〄"
    )
    LOGS.info(suc_msg)
